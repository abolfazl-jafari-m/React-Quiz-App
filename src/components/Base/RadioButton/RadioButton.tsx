...
function RadioButton({label , id , value , name , onChange , variant = 'purple' ,checked} : IProps) {
    const theme : any = {
    ...
    return (
        <div className={`relative ${theme[variant]}`}>
            ...
            <input type={"radio"} value={value} name={name} id={id} className={"appearance-none absolute w-full h-full top-0 left-0 z-10 cursor-pointer"} onChange={onChange} checked={checked}/>
        </div>
    );
}
